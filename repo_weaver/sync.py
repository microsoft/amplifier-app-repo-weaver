...
``_sources/*-changes.md`` filenames to recover (a) the last-sync date and
(b) the set of already-tracked ``(owner, repo)`` pairs, asks GitHub (via
``gh``) which of those repos have pushed since, ensures a local clone for
each changed repo, and re-weaves each one over the reused single-repo
:func:`repo_weaver.weave.weave` path.
...
# ---------------------------------------------------------------------------
# Corpus introspection: last-sync date + tracked (owner, repo) set
# ---------------------------------------------------------------------------
...
def _tracked_repos(corpus_path: Path) -> tuple[set[tuple[str, str]], set[str]]:
    ...
    Only qualifiers containing the ``owner__repo`` double-underscore form are
    tracked — the no-remote fallback (bare basename) has no owner to query
    ...
    for qualifier, _date_str in _iter_source_change_files(corpus_path):
        if "__" not in qualifier:
            continue
        owner, _, repo = qualifier.partition("__")
        if owner and repo:
            tracked.add((owner, repo))
...
def sync_corpus(
    ...
    """Re-weave only the repos that changed since the corpus's last sync.
    ...
    1. Determines the last-sync date — *since* if given, else the max
       ``YYYY-MM-DD`` parsed across every ``_sources/*-changes.md`` filename.
    ...
    3. For each owner, asks GitHub (``gh repo list``) for every non-fork
       ...
       is CHANGED if ``pushedAt`` is strictly after the last-sync date.
    4. Unless *dry_run*, ensures a local clone of each changed repo under
       *clones_dir* and re-weaves it (reusing the existing single-repo
       :func:`repo_weaver.weave.weave` path) over the window
       ``(last_sync, until]``.
    ...
    Args:
        ...
        since:       Override for the last-sync date (``YYYY-MM-DD``,
                     exclusive). Defaults to the corpus-derived value.
    ...
    Returns:
        A result dict:
            ``last_sync``   (str)              — the resolved last-sync date.
            ``until``       (str)               — the resolved window end.
            ``owners``      (dict[str, int])    — per-owner changed-repo counts.
            ``changed``     (list[dict])        — changed repos: each has
                                                   ``owner``, ``repo``,
                                                   ``nameWithOwner``, ``pushedAt``.
            ``errors``      (list[str])         — non-fatal owner-level notes
                                                   (e.g. gh returned nothing for
                                                   an owner with tracked repos).
    ...
    for owner in sorted(owners):
        repos_info = gitio.gh_list_repos(owner)
        ...
        for r in repos_info:
            ...
            if pushed_date and pushed_date > last_sync:
