...
repo-weaver's build-time hatchling hook (``hatch_build.py`` at the repo
root) bakes a git-derived version string into ``repo_weaver/_version.py``
before a wheel is built (see that file's docstring for the full rationale:
the package is installed from a git source, ``.git`` is present at build
time but never ships in the wheel, so the value must be computed once, at
build, and read as a plain string at runtime).
...
2. **Local dev checkout run directly** (``uv run repo-weaver --version``,
...
Mirrors wiki-weaver's own ``wiki_weaver/_version_resolve.py`` -- same
resolution logic, adapted to repo-weaver's module layout.
"""
...
# repo_weaver/_version_resolve.py -> repo_weaver/ -> repo root
...
def resolve_version(baked: str, *, root: Optional[Path] = None) -> str:
    ...
    Args:
        baked: the on-disk ``repo_weaver.__version__`` value, as imported
            from ``_version.py``.
