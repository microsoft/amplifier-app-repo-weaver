...
Mirrors wiki-weaver's own eval/test_hatch_build.py -- same coverage,
...
# the repo root, not part of the repo_weaver package).
...
class TestInitialize:
    def test_success_writes_version_file_and_logs_info(self, tmp_path):
        pkg_dir = tmp_path / "repo_weaver"
        pkg_dir.mkdir()
        version_file = pkg_dir / "_version.py"
        version_file.write_text('__version__ = "0.1.0"\n', encoding="utf-8")

        hook, app = _make_hook(tmp_path)
        with patch.object(hook, "_compute_version", return_value="2026.07.08-a1b2c3d"):
            hook.initialize("0.1.0", {})

        content = version_file.read_text(encoding="utf-8")
        assert '__version__ = "2026.07.08-a1b2c3d"' in content
