...
repo-weaver is installed via ``uv tool install git+https://...`` -- a
...
``repo_weaver/_version.py`` with that value before the build packages files
...
no SHA pinning" versioning model (see the ``wiki-weaver @ git+...@main``
dependency pin in pyproject.toml, and repo_weaver/updater.py).
...
``repo_weaver`` package. The package being built is not guaranteed
...
The runtime dev-mode fallback (``repo_weaver/_version_resolve.py``) uses the
same two git commands independently -- see that module's docstring.

This mirrors wiki-weaver's own ``hatch_build.py`` (same mechanism, proven
end-to-end via a real ``uv tool install git+file://...`` cycle) -- see that
file for the sibling implementation this was adapted from.
"""
...
_VERSION_FILE_RELATIVE = Path("repo_weaver") / "_version.py"


class GitVersionBuildHook(BuildHookInterface):
    """Overwrites repo_weaver/_version.py with a git-derived version at build time."""
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        if computed is None:
            self.app.display_warning(
                ...
                "the existing on-disk repo_weaver/_version.py unchanged."
            )
            return

        version_path = Path(self.root) / _VERSION_FILE_RELATIVE
        version_path.write_text(
            '"""Single source of truth for the repo-weaver package version.\n'
            "\n"
            ...
            "the repo_weaver package __init__.\n"
            "\n"
